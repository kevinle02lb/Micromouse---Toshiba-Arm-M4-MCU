...
#define SYS_CG_PRCK_FC_1            (0x00000000UL)      /*!< CG ��T0 fc register value */
#define SYS_CG_PRCK_FC_2            (0x00000100UL)      /*!< CG ��T0 fc/2 register value */
#define SYS_CG_PRCK_FC_4            (0x00000200UL)      /*!< CG ��T0 fc/4 register value */
#define SYS_CG_PRCK_FC_8            (0x00000300UL)      /*!< CG ��T0 fc/8 register value */
#define SYS_CG_PRCK_FC_16           (0x00000400UL)      /*!< CG ��T0 fc/16 register value */
#define SYS_CG_PRCK_FC_32           (0x00000500UL)      /*!< CG ��T0 fc/32 register value */
#define SYS_CG_PRCK_FC_64           (0x00000600UL)      /*!< CG ��T0 fc/64 register value */
#define SYS_CG_PRCK_FC_128          (0x00000700UL)      /*!< CG ��T0 fc/128 register value */
#define SYS_CG_PRCK_FC_256          (0x00000800UL)      /*!< CG ��T0 fc/256 register value */
#define SYS_CG_PRCK_FC_512          (0x00000900UL)      /*!< CG ��T0 fc/512 register value */

#define SYS_CG_PLL0SEL_PLL0ON_SET   ((uint32_t)0x00000001)

#define SYS_CG_MCKSEL_1             (0x00000000UL)      /*!< CG fsysm, m��T0 fc value */
#define SYS_CG_MCKSEL_2             (0x00000040UL)      /*!< CG fsysm, m��T0 fc/2 value */
#define SYS_CG_MCKSEL_4             (0x00000080UL)      /*!< CG fsysm, m��T0 fc/4 value */
...
#if defined ( __CC_ARM ) || (defined(__ARMCC_VERSION) && (__ARMCC_VERSION >= 6000000)) /* RealView Compiler */
...
/* System clock is high-speed clock*/
#if (CLOCK_SETUP)
    #define CORE_TALH (EXTALH)            /* External high-speed oscillator */
#else
    #define CORE_TALH (IXTALH)            /* Internal high-speed oscillator (10MHz) */
#endif

#if ((PLL0SEL_Val & (1U<<1U)) && (PLL0SEL_Val & (1U<<0U)))  /* If PLL selected and enabled */
    #if (CLOCK_SETUP)                                       /* External oscillator path */
    ...
    #else                                                   /* Internal oscillator path */
        #if (CORE_TALH == IOSC_10M)                         /* If input is 10MHz */
            #if ((PLL0SEL_Val & PLL0SEL_MASK) == SYS_CG_10M_MUL_16_FPLL)
...
{                               
    ...
    if (oscsel) 
    {  
        /* If system clock is External high-speed oscillator freq */
        CoreClock = EXTALH;
    } 
    else 
    {                    
        /* If system clock is Internal high-speed oscillator freq */
        CoreClock = IXTALH;
    }
...
/* Warm-up */
    ...
    /* Optional: MCU automatically uses Internal HOSC as system clock after reset, so the following steps are optional 
    if user want to use Internal HOSC as system clock.*/
    /* NOTHING TO DO — already running at 10MHz */

    /* Configure CG OSCCR */
    TSB_CG->OSCCR |= CG_OSCCR_IHOSC1EN_SET;    /* Set Internal HOSC Enable bit */


    /* 
    * Optional: Set the Warm-up Time 
    * Refer to Datasheet for details about the relation between Warm-up Time and Clock Frequency.
    * 
    * 1.2.4.2. Directions for Warming-up Timer (Pg. 14)
    * 
    */

    // 1. Selection of Clock Source
    TSB_CG->WUPHCR &= ~CG_WUPHCR_WUCLK_MASK;   /* Clear for Internal HOSC */
...
/* Middle Speed System Clock Setup */
